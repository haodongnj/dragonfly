...
auto OpPopCount(const ZSetFamily::ZRangeSpec& range_spec, const OpArgs& op_args, string_view key)
  ...
  auto zlen = pv.Size();
  if (zlen == 0) {
    CHECK(op_args.db_cntx.tenant->GetCurrentDbSlice().Del(op_args.db_cntx.db_index, res_it->it));
  }
...
void ZSetFamily::ZCard(CmdArgList args, ConnectionContext* cntx) {
  ...
  auto cb = [&](Transaction* t, EngineShard* shard) -> OpResult<uint32_t> {
    auto find_res =
        t->GetTenant().GetCurrentDbSlice().FindReadOnly(t->GetDbContext(), key, OBJ_ZSET);
    if (!find_res) {
      return find_res.status();
    }
