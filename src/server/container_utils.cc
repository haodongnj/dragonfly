...
OpResult<string> RunCbOnFirstNonEmptyBlocking(Transaction* trans, int req_obj_type,
  ...
  auto tenant = &trans->GetTenant();
  const auto key_checker = [req_obj_type, tenant](EngineShard* owner, const DbContext& context,
                                                  Transaction*, std::string_view key) -> bool {
    return tenant->GetCurrentDbSlice().FindReadOnly(context, key, req_obj_type).ok();
  };
